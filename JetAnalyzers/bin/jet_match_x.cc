...
public:
   ...
   void   SetJEC(string JECPar = "ParallelMCL1_L1FastJet_AK4PFchs.txt");
...
private:
   ...
   ofstream eventlist;
//   ofstream LargeOffset;
//   ofstream LargeOffset_APV;
...
void MatchEventsAndJets::DeclareHistograms(bool reduceHistograms) {
   ...
   histograms["npus_vs_tnpus"]		     = new TH2D("npus_vs_tnpus","npus_vs_tnpus;Reco N_{PU};True N_{PU}",NNPU,vnpu,NTNPU,vtnpu);	
   histograms["npus_vs_tnpus_weighted"]	     = new TH2D("npus_vs_tnpus_weighted","npus_vs_tnpus_weighted;Reco N_{PU};True N_{PU}",NNPU,vnpu,NTNPU,vtnpu);
  ...
  for (int det=0;det<NDetectorNames;det++) {
      ...
      hname = Form("jetpt_pu_%s_npu%i_%i",detectorAbbreviation.Data(),0,
                   ((NBinsNpvRhoNpu-1)*npvRhoNpuBinWidth)+npvRhoNpuBinWidth-1);
      histograms[hname] = new TH1D(hname,hname+";jet p_{T}^{PU};",NPtBins, vpt);

      hname = Form("jetpt_nopu_%s_npu%i_%i",detectorAbbreviation.Data(),0,
                   ((NBinsNpvRhoNpu-1)*npvRhoNpuBinWidth)+npvRhoNpuBinWidth-1);
      histograms[hname] = new TH1D(hname,hname+";jet p_{T}^{EpsilonPU};",NPtBins, vpt);
      ...
      for (int npv=0;npv<NBinsNpvRhoNpu;npv++) {
         ...
         hname = Form("p_offresVsrefpt_%s_tnpu%i_%i",detectorAbbreviation.Data(),
                      npv*npvRhoNpuBinWidth,npv*npvRhoNpuBinWidth+npvRhoNpuBinWidth-1);
         histograms[hname] = new TH2D(hname,hname+";p^{GEN}_{T}; p_{T}/p_{T}^{nopu};",NPtBins, vpt,6000,-300,300);
	 ...
	 //To obtain the jet pt from the PU file as a function of NPU
	 hname = Form("jetpt_pu_%s_npu%i_%i",detectorAbbreviation.Data(),npv*npvRhoNpuBinWidth,npv*npvRhoNpuBinWidth+npvRhoNpuBinWidth-1);
         histograms[hname] = new TH1D(hname,hname+";jet p_{T}^{PU};",NPtBins, vpt);

	 //To obtain the jet pt from the EpsilonPU (or NoPU) file as a function of NPU
	 hname = Form("jetpt_nopu_%s_npu%i_%i",detectorAbbreviation.Data(),npv*npvRhoNpuBinWidth,npv*npvRhoNpuBinWidth+npvRhoNpuBinWidth-1);
         histograms[hname] = new TH1D(hname,hname+";jet p_{T}^{EpsilonPU};",NPtBins, vpt);
...
//   LargeOffset.open("/afs/cern.ch/work/i/izisopou/public/JEC/CMSSW_10_5_0/src/LargeOffset.txt");
//   LargeOffset_APV.open("/afs/cern.ch/work/i/izisopou/public/JEC/CMSSW_10_5_0/src/LargeOffset_APV.txt");
...
bool MatchEventsAndJets::FillHistograms(bool reduceHistograms, IT::const_iterator it) {
   ...
   double offset = 0, offset_raw = 0, offsetOA = 0, offsetOrefpt = 0, areaDiff = 0, resp = 0,
          respTonopu = 0, respNopu = 0, PUEff = 0, GenSumPtOA = 0, jetpt_pu = 0, jetpt_nopu = 0;
   int diff_pdgid = 0;
...
//cout << "JetPhi" << tpu->jtphi->at(jpu) << endl;
      ...
      jetpt_pu	    = tpu->jtpt->at(jpu);
      jetpt_nopu    = tnopu->jtpt->at(jnopu);
...
//      if (offset>200 || offset<-200) LargeOffset<<tpu->run<<":"<<tpu->lumi<<":"<<tpu->evt<<"\t"<<tnopu->run<<":"<<tnopu->lumi<<":"<<tnopu->evt<<"\t"<<offset<<endl;

/*      if (offset>200 || offset<-200) 
      {
		if((tpu->run == tnopu->run) && (tpu->lumi == tnopu->lumi) && (tpu->evt == tnopu->evt))
		{
			LargeOffset<<tpu->run<<":"<<tpu->lumi<<":"<<tpu->evt<<"\t"<<offset<<endl;
			LargeOffset_APV<<tpu->run<<":"<<tpu->lumi<<":"<<tpu->evt<<endl;
		}
		else
		{
			LargeOffset<<tpu->run<<":"<<tpu->lumi<<":"<<tpu->evt<<"\t"<<tnopu->run<<":"<<tnopu->lumi<<":"<<tnopu->evt<<"\t"<<offset<<endl;
			LargeOffset_APV<<tpu->run<<":"<<tpu->lumi<<":"<<tpu->evt<<"\t"<<tnopu->run<<":"<<tnopu->lumi<<":"<<tnopu->evt<<endl;
		}
      }
*/
...
//mu<40 cut for the APV UL16 MC
/*      if (tpu->tnpus->at(iIT)<40) dynamic_cast<TProfile3D*>(histograms["p_offOverA_etaVsTnpusVsJetPt"])->Fill(tpu->jteta->at(jpu),tpu->tnpus->at(iIT),tpu->refpt->at(jpu),offsetOA,weight);
      if (tpu->tnpus->at(iIT)<40) dynamic_cast<TProfile3D*>(histograms["p_PtAve_etaVsTnpusVsJetPt"])   ->Fill(tpu->jteta->at(jpu),tpu->tnpus->at(iIT),tpu->refpt->at(jpu),tpu->jtpt->at(jpu),weight);
      if (tpu->tnpus->at(iIT)<40) dynamic_cast<TProfile3D*>(histograms["p_RhoAve_etaVsTnpusVsJetPt"])  ->Fill(tpu->jteta->at(jpu),tpu->tnpus->at(iIT),tpu->refpt->at(jpu),tpu->rho,weight);
      if (tpu->tnpus->at(iIT)<40) dynamic_cast<TH3I*>(histograms["p_Events_etaVsTnpusVsJetPt"])  ->Fill(tpu->jteta->at(jpu),tpu->tnpus->at(iIT),tpu->refpt->at(jpu));
*/
      ...
      dynamic_cast<TH2D*>(histograms["npus_vs_tnpus"]) 		->Fill(tpu->npus->at(iIT),tpu->tnpus->at(iIT));
      dynamic_cast<TH2D*>(histograms["npus_vs_tnpus_weighted"]) ->Fill(tpu->npus->at(iIT),tpu->tnpus->at(iIT),weight);
      ...
      hname = Form("jetpt_pu_%s_npu%i_%i",detectorAbbreviation.Data(),inpu_low,inpu_high);
      dynamic_cast<TH1D*>(histograms[hname])->Fill(jetpt_pu,weight);
      hname = Form("jetpt_nopu_%s_npu%i_%i",detectorAbbreviation.Data(),inpu_low,inpu_high);
      dynamic_cast<TH1D*>(histograms[hname])->Fill(jetpt_nopu,weight);
...
int main(int argc,char**argv)
{
   ...
   bool         ApplyJEC          = cl.getValue<bool>    ("ApplyJEC",                              true);
   string       JECpar            = cl.getValue<string>  ("JECpar",               "ParallelMCL1_L1FastJet_AK4PFchs.txt");
