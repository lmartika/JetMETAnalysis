...
int main(int argc,char**argv)
{
  ...
  TString         weightfilename    = cl.getValue<TString>      ("weightfilename",       "");
   ...
   for(unsigned int a=0; a<algs.size(); a++) {
	...
	int lootnpu=0;
        double sumpt=0;
	if(iIT>0)sumpt = JRAEvt->sumpt_lowpt->at(1);
        float pthat = JRAEvt->pthat;
        float evt_fill = true;

        if(iIT>0){
	npu = sumEOOT(JRAEvt->npus,iIT)+JRAEvt->npus->at(iIT)+sumLOOT(JRAEvt->npus,iIT);
	eootnpu = (int)sumEOOT(JRAEvt->npus,iIT);
	itnpu = JRAEvt->npus->at(iIT);
        lootnpu = (int)sumLOOT(JRAEvt->npus,iIT);
	...
	//	cout<<"point one"<<endl;
	if (dphimin>0 && abs(JRAEvt->jtphi->at(0)-JRAEvt->jtphi->at(1))<dphimin){
	  cout<<"WARNING::Something happens with the dphimin or jet phi"<<endl;
	  continue;}

	if (pthatmin>0.0 && pthat<pthatmin) {
	 cout << "WARNING::The pthat of this event is less than the minimum pthat!" << endl;
	  continue;
	}
	if (pthatmax!=-1.0 && pthat>pthatmax) {
	  cout << "WARNING::The pthat of this event is greater than the maximum pthat!" << endl;
            continue;
	}
    ...
    //if(JRAEvt->tnpus->at(iIT)>=40) continue;
...
//    if(JRAEvt->npv>0 && JRAEvt->npv<10){  //nVtx cut starts here
	   ...
	   if(iIT>0){
             rho = JRAEvt->rho;
             rho_hlt = (0!=chain->GetBranch("rho_hlt")) ? JRAEvt->rho_hlt : 0;}
	    ...
	    //cout<<"this is jet pT "<<pt<<" "<<ptgen<<" "<<eta<<endl; 
            ...
            if(JetCorrector) {
               ...
               if (TString(JetInfo::get_correction_levels(levels,L1FastJet)).Contains("L1FastJet")) {
                  ...
                  if (jetInfo.isHLT()){ cout<<"it is hlt"<<endl;
		    JetCorrector->setRho(rho_hlt);}
                  else
	    ...
	    //cout<<"after jet correction"<<endl;
            //
		...
		cout << "getBin(fabs(eta),veta_coarse,NETA_Coarse)-4=" << getBin(fabs(eta),veta_coarse,NETA_Coarse)-(NETA_Coarse/2) << endl;
	      ...
	      if(iIT>0) TPUDistribution->Fill(JRAEvt->tnpus->at(iIT),weight);
            }
