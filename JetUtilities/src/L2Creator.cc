...
void L2Creator::loopOverAlgorithms(string makeCanvasVariable) {
    ...
    while ((dirKey=(TKey*)nextDir())) {
        ...
        cout<<"after asserting checkFormulaEvaluator"<<endl;
        //
...
//Initial parameters for std+gaus fit for all eta bins are in L1141-1177. Here we can change these initial parameters for specific eta bins if you see that the fits have failed. Commented out for now, but one needs to identify which of the 82 fits fail and try to fix them by changing the parameters below

/*			if(ieta==7 || ieta==19 || (ieta>=29 && ieta<=40) || (ieta>=47 && ieta<=52) || ieta==55 || ieta==57 || ieta==59 || ieta==62 || ieta==65)
			{
			...
			if(ieta==21 || ieta==58 || (ieta>=23 && ieta<=25) || ieta==28 || ieta==53 || ieta==56)
			{
				fabscor->SetParameter(0,-0.0221278);
        			fabscor->SetParLimits(0,-2,25);
        			fabscor->SetParameter(1,1.2);
			...
			if((ieta>=41 && ieta<=46) || ieta==63 || ieta==67)
			{
				fabscor->SetParameter(0,-0.0221278);
        			fabscor->SetParLimits(0,-2,25);
        			fabscor->SetParameter(1,30);
        			fabscor->SetParLimits(1,0,100);
        			fabscor->SetParameter(2,100);
        			fabscor->SetParLimits(2,0,200);
        			fabscor->SetParameter(3,-0.0679365);
        			fabscor->SetParLimits(3,-15,15);
			...
			if(ieta==28 || ieta==56)
			{
				fabscor->SetParameter(0,-0.0221278);
        			fabscor->SetParLimits(0,-2,25);
        			fabscor->SetParameter(1,1.2);
        			fabscor->SetParLimits(1,-100,400);
        			fabscor->SetParameter(2,100);
        			fabscor->SetParLimits(2,-100,200);
        			fabscor->SetParameter(3,-0.0679365);
        			fabscor->SetParLimits(3,-50,50);
        			fabscor->SetParameter(4,2.82597);
       				fabscor->SetParLimits(4,0,500);
        			fabscor->SetParameter(5,1.8277);
        			fabscor->SetParLimits(5,-20,50);
...
*/
			...
			if(alg.find("puppi")!=string::npos){ //Eirini
    ...
    g_chi2ndf_vs_eta->GetYaxis()->SetTitle("#chi^{2}/ndf");
    g_chi2ndf_vs_eta->GetXaxis()->SetTitle("#eta");
    g_chi2ndf_vs_eta->SetMarkerStyle(20);
    g_chi2ndf_vs_eta->GetYaxis()->SetRangeUser(0.,10.);
    ...
    for (unsigned int ieta=0;ieta<vector_size;ieta++) {
        ...
        if(root_func!=nullptr) {
	      ...
	      if (ipt<ptmin || ipt>ptmax){
                //cout<<"outside limits "<<ipt<<" "<<ptmin<<" "<<ptmax<<endl;
		continue;}
...
void L2Creator::writeTextFileForCurrentAlgorithm() {
    ...
    for (unsigned int ieta=0;ieta<vector_size;ieta++) {
        ...
        if(frelcor!=0) {
            ...
            cout<<"en write text file for current algorithm"<<ptmin<<" "<<grelcor<<" "<<frelcor->GetNpar()+2<<endl;
...
void L2Creator::writeTextFileForCurrentAlgorithm_spline() {
    ...
    for (unsigned int ieta=0;ieta<vector_size;ieta++) {
		...
		cout<<"en write text file for current algorith spline"<<" "<<grelcor<<" "<<frelcor->GetNpar()+2<<endl;
